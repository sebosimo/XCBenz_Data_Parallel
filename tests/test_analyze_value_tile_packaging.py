...
import analyze_value_tile_packaging as MODULE
...
def dataset(name: str, width: int, height: int, values: bytes, *, cloud: bool = False):
    return MODULE.Dataset(
        ...
        encoding_format="packed-uint4-cloud-cover"
        if cloud
        else "uint8-interleaved-components",
...
class ValueTilePackagingTests(unittest.TestCase):
    def test_cloud_pack_round_trip_preserves_codes_and_odd_padding(self):
        codes = bytes([0, 1, 10, 15, 7])
        packed = MODULE.pack_cloud_codes(codes, 15)
        self.assertEqual(packed, bytes([16, 250, 247]))
        self.assertEqual(MODULE.unpack_cloud_codes(packed, len(codes)), codes)

    def test_window_has_one_cell_halo_and_missing_domain_padding(self):
        source = dataset("rain", 3, 2, bytes([1, 2, 3, 4, 5, 6]))
        window = source.encode_window(-1, -1, 4, 4)
        self.assertEqual(
            window,
            bytes([255, 255, 255, 255, 255, 1, 2, 3, 255, 4, 5, 6, 255, 255, 255, 255]),
        )

    def test_selector_uses_core_tiles_while_payload_halo_covers_edges(self):
        grid = MODULE.Grid(10, 8, 4.0, 43.0, 0.02, 0.02)
        bbox = (4.02, 43.02, 4.11, 43.11)
        self.assertEqual(
            MODULE.tiles_for_bbox(grid, (4, 4), bbox), {(0, 0), (1, 0), (0, 1), (1, 1)}
        )
    ...
    def test_hybrid_cloud_group_preserves_single_layer_bytes_and_reduces_stack_requests(
        ...
        hybrid_stack = candidate["hybrid_cloud_channels"]["cloud_stack"]["views"][
            "alps"
        ]
        ...
        self.assertLess(hybrid_stack["requests"], ordinary_stack["requests"])
        self.assertEqual(
            hybrid_low["compressed_bytes"], ordinary_low["compressed_bytes"]
        )

    def test_edge_padding_flag_covers_domain_halo_but_not_interior_tiles(self):
        ...
        self.assertTrue(edge_flags & 1)
        self.assertFalse(interior_flags & 1)
