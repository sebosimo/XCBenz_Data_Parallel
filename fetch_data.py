...
def main():
    ...
    if force_refresh:
        log("FORCE_REFRESH was selected by orchestration; regenerating the requested CH1 range.", "NOTICE")
    log(
    ...
    runs = [pinned_run] if pinned_run is not None else get_latest_available_runs(
        limit=CH1_POLICY.processing_candidate_limit
    )
    ...
    for ref_time in runs:
        ...
        max_h = CH1_POLICY.maximum_horizon(ref_time)
        ...
        def fetch_horizon_batch(h_value):
            plan = CH1_POLICY.horizon_plan(h_value, profile_mode=profile_mode, products=runtime_products)
        ...
        for h in range(horizon_start, run_horizon_end + 1):
            ...
            has_new_data = False
            plan = CH1_POLICY.horizon_plan(h, profile_mode=profile_mode, products=runtime_products)
